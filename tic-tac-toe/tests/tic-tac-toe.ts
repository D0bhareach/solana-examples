...
// utility function to construct Transaction.
...
async function getAnchorProvider(file:string, cluster: any): Promise<anchor.AnchorProvider> {
  ...
  const connection: Connection = new Connection(clusterApiUrl(cluster), commitment);
...
describe('tic-tak-toe', () => {
  ...
  beforeEach(async function(): Promise<void> {
        ...
        program = new Program(IDL, new PublicKey("DFUVqtTDCXTywA7ELfN6WwXv6Gcw7UqK2WkVJK8rvzd7"), programProvider);
  ...
  it('player one wins!', async () => {
    ...
    try {
      ...
      // if add check for error type here to be AnchorError it will fail
      const err: AnchorError = _err;
